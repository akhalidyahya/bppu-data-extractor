...
function extractHeaderMetadata($text) {
    $pattern = '/([A-Z0-9]{9})(\d{2}-\d{4})(TIDAK FINAL|FINAL)(NORMAL|PEMBETULAN)/i';
    if (preg_match($pattern, $text, $matches)) {
        return [
            'nomor' => $matches[1],
            'masa_pajak' => $matches[2],
            'sifat_pemotongan' => strtoupper($matches[3]),
            'status_bukti' => strtoupper($matches[4]),
        ];
    }
...
try {
    ...
    foreach (glob("$tempDir/*.pdf") as $pdfPath) {
        ...
        $header_data = extractHeaderMetadata($text);
        $h1 = $header_data['nomor'];
        $h2 = $header_data['masa_pajak'];
        $h3 = $header_data['sifat_pemotongan'];
        $h4 = $header_data['status_bukti'];
